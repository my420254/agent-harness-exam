...
## 四、真实 API 实测（deepseek-v4-flash 会犯的错 + 如何被接住）

用真实 flash 跑刁难任务，模型确实会犯两类错，runtime 全部接住并恢复（`demo_errors.py` 可一键复现）：

**1. 不按 JSON 约定输出 → parse_error → 自动重试**
- 触发「直接回答 1+1，不要用 JSON」：模型听话输出纯文本 `2` → 解析失败 → runtime 把"输出格式错误"塞回上下文 → 模型重试后正常给出答案。
- 触发「算'苹果加香蕉'」：模型前两轮都输出非 JSON → 连续两次 parse_error → 重试后收敛。

**2. 工具参数错误 → tool_error → 结果回喂 → 模型自行修正**
- 触发「算'苹果加香蕉'」：模型调 `search` 时 `query` 为空 → 抛 `query is required` → runtime 把错误回喂 → 模型改用正确参数再调，最终给出合理回答。

**3. 读懂工具 Schema 并自适应（不犯错，但值得记录）**
- 触发「用 delete 动作记待办」：模型发现 todo 只支持 add/list/clear，自动改用 `add` 并解释限制。

结论：这些"错误"正是最小 Agent 必须兜住的场景。它们既能在真机上复现（`demo_errors.py`），也有对应的确定性测试用例（`test_runtime.py` 的 parse_error 重试、tool_error 继续），两头都对得上。
